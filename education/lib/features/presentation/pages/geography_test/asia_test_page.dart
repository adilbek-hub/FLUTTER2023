...
class AsiaTestPage extends StatefulWidget {
  const AsiaTestPage({super.key});

  @override
  State<AsiaTestPage> createState() => _AsiaTestPageState();
}

class _AsiaTestPageState extends State<AsiaTestPage> {
  int indexAsia = 0;
  int tuuraJooptor = 0;
  int kataJooptor = 0;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AsiaTestBloc, EducationState>(
      builder: (context, state) {
        if (state is EducationLoading) {
          return const LoadingWidget();
        } else if (state is AsiaTestSuccess) {
          return SafeArea(
            child: Scaffold(
              appBar: AppBar(
                leading: IconButton(
              ...
              body: Column(
                children: [
                  SliderWidget(max: 12, valueIndex: indexAsia.toDouble()),
                  Padding(
                    ...
                    child: Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: CachedNetworkImage(
                          imageUrl: state.asiaTestToicsModel[indexAsia].image,
                          fit: BoxFit.cover,
                      ...
                      itemBuilder: ((context, index) {
                        return Card(
                          ...
                          child: InkWell(
                            onTap: () {
                              if (indexAsia + 1 ==
                                  state.asiaTestToicsModel.length) {
                                showDialog<String>(
                                  context: context,
                                ...
                                if (state.asiaTestToicsModel[indexAsia]
                            ...
                            child: Center(
                              child: AutoSizeText(
                                state.asiaTestToicsModel[indexAsia]
