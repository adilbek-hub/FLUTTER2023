...
class KishiJanybarTestPage extends StatefulWidget {
  const KishiJanybarTestPage({super.key});
...
class _KishiJanybarTestPageState extends State<KishiJanybarTestPage> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ManAndAnimalTestBloc, EducationState>(
      builder: (context, state) {
        if (state is EducationLoading) {
          return const LoadingWidget();
        } else if (state is ManAndAnimalsTestSuccess) {
          return SafeArea(
            child: Scaffold(
              appBar: AppBar(
                leading: IconButton(
              ...
              body: Column(
                children: [
                  SliderWidget(max: 7, valueIndex: indexadamJanybar.toDouble()),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 10),
                    child: Center(
                      child: Text(
                        state.manAndAnimalsTestToicsModel[indexadamJanybar]
                            .guestion,
                    ...
                    child: Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: CachedNetworkImage(
                          imageUrl: state
                              .manAndAnimalsTestToicsModel[indexadamJanybar]
                              .image,
                          fit: BoxFit.cover,
                          ...
                          child: InkWell(
                            onTap: () {
                              if (indexadamJanybar + 1 ==
                                  state.manAndAnimalsTestToicsModel.length) {
                                showDialog<String>(
                                  context: context,
